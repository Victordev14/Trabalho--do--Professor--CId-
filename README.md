...
| ------------------------------------------- | ----------------------------------- | ---------------------------------------------- |
| id\_cliente                                 | Identificador conceitual do cliente | Utilizado para identificar o cliente no modelo |
...
| AtributoDescriçãoRegra de negócio associada |                                         |                                                    |
| ------------------------------------------- | --------------------------------------- | -------------------------------------------------- |
| id\_atendimento                             | Identificador conceitual do atendimento | Utilizado para identificar o atendimento no modelo |
| data                                        | Data do atendimento                     | Registra quando o atendimento ocorreu              |
| tipo\_atendimento                           | Tipo do atendimento realizado           | Identifica o tipo de atendimento                   |

### Receita

| AtributoDescriçãoRegra de negócio associada |                                              |                                                |
| ------------------------------------------- | -------------------------------------------- | ---------------------------------------------- |
| id\_receita                                 | Identificador conceitual da receita          | Utilizado para identificar a receita no modelo |
...
| AtributoDescriçãoRegra de negócio associada |                                         |                                                    |
| ------------------------------------------- | --------------------------------------- | -------------------------------------------------- |
| id\_funcionario                             | Identificador conceitual do funcionário | Utilizado para identificar o funcionário no modelo |
...
| AtributoDescriçãoRegra de negócio associada |                                   |                                              |
| ------------------------------------------- | --------------------------------- | -------------------------------------------- |
| id\_venda                                   | Identificador conceitual da venda | Utilizado para identificar a venda no modelo |
| data                                        | Data da venda                     | Registra quando a venda ocorreu              |
| valor                                       | Valor da venda                    | Deve ser registrado                          |
| numero\_nota                                | Número da nota da venda           | Relacionado ao registro da venda             |
| prazo\_garantia                             | Prazo de garantia da venda        | A garantia informada é de 1 ano              |

### Item\_Venda

| AtributoDescriçãoRegra de negócio associada |                                           |                                             |
| ------------------------------------------- | ----------------------------------------- | ------------------------------------------- |
| id\_item\_venda                             | Identificador conceitual do item da venda | Utilizado para identificar o item no modelo |
| quantidade                                  | Quantidade do produto na venda            | Registra a quantidade vendida               |
| valor\_unitario                             | Valor unitário do produto na venda        | Registra o valor unitário                   |
...
| AtributoDescriçãoRegra de negócio associada |                                        |                                                  |
| ------------------------------------------- | -------------------------------------- | ------------------------------------------------ |
| id\_pagamento                               | Identificador conceitual do pagamento  | Utilizado para identificar o pagamento no modelo |
| forma\_pagamento                            | Forma utilizada no pagamento           | Pode ser cartão de crédito, PIX ou dinheiro      |
...
| AtributoDescriçãoRegra de negócio associada |                                              |                                              |
| ------------------------------------------- | -------------------------------------------- | -------------------------------------------- |
| id\_ordem                                   | Identificador conceitual da ordem de serviço | Utilizado para identificar a ordem no modelo |
| status                                      | Situação da ordem de serviço                 | Permite acompanhar o andamento do serviço    |
| data\_previsao                              | Data prevista para finalização/entrega       | Utilizada no acompanhamento da ordem         |
| data\_entrega                               | Data de entrega                              | Registra a entrega da ordem de serviço       |

### Produto

| AtributoDescriçãoRegra de negócio associada |                                          |                                                                    |
| ------------------------------------------- | ---------------------------------------- | ------------------------------------------------------------------ |
| id\_produto                                 | Identificador conceitual do produto      | Utilizado para identificar o produto no modelo                     |
...
| tipo\_montagem                              | Tipo de montagem relacionado ao produto  | Quando aplicável                                                   |
...
| quantidade\_disponivel                      | Quantidade disponível do produto         | Controlada por contagem                                            |

### Fornecedor

| AtributoDescriçãoRegra de negócio associada |                                        |                                                   |
| ------------------------------------------- | -------------------------------------- | ------------------------------------------------- |
| id\_fornecedor                              | Identificador conceitual do fornecedor | Utilizado para identificar o fornecedor no modelo |
...
| AtributoDescriçãoRegra de negócio associada |                                    |                                               |
| ------------------------------------------- | ---------------------------------- | --------------------------------------------- |
| id\_compra                                  | Identificador conceitual da compra | Utilizado para identificar a compra no modelo |
...
| AtributoDescriçãoRegra de negócio associada |                                            |                                             |
| ------------------------------------------- | ------------------------------------------ | ------------------------------------------- |
| id\_item\_compra                            | Identificador conceitual do item da compra | Utilizado para identificar o item no modelo |
| quantidade                                  | Quantidade do produto comprado             | Registra a quantidade comprada              |
| valor\_unitario                             | Valor unitário do produto comprado         | Registra o valor unitário da compra         |
...
- **Entidades reconhecidas:** Cliente, Atendimento, Receita, Funcionário, Venda, Item\_Venda, Pagamento, Ordem\_Servico, Produto, Fornecedor, Compra e Item\_Compra. Essas entidades representam os principais dados e objetos identificados nos processos levantados na entrevista e estruturados no modelo conceitual.

- **Atributos e classificações:** Cliente possui nome, CPF, endereço, CEP e número. Atendimento possui data e tipo de atendimento. Receita possui data, grau, DNP e altura. Funcionário possui nome. Venda possui data, valor, número da nota e prazo de garantia. Item\_Venda possui quantidade, valor unitário e desconto. Pagamento possui forma de pagamento, valor e parcelamento. Ordem\_Servico possui status, data de previsão e data de entrega. Produto possui tipo, referência, marca, material, tipo de montagem, tratamento, medida, grau, bloco, índice e quantidade disponível, conforme aplicável. Fornecedor possui nome e localização. Compra possui data e status. Item\_Compra possui quantidade e valor unitário. Os identificadores conceituais são utilizados para individualizar os registros no modelo.
...
- **Cardinalidades:** Cliente (0,n) — REALIZA — (1,1) Atendimento; Funcionário (0,n) — REALIZA — (1,1) Atendimento; Atendimento (0,1) — GERA — (1,1) Receita; Cliente (0,n) — POSSUI — (1,1) Receita; Cliente (0,n) — REALIZA — (1,1) Venda; Funcionário (0,n) — EFETUA — (1,1) Venda; Venda (1,n) — CONTEM — (1,1) Item\_Venda; Venda (1,n) — RECEBE — (1,1) Pagamento; Venda (0,1) — ACOMPANHA — (1,1) Ordem\_Servico; Receita (0,n) — BASEIA — (0,1) Item\_Venda; Item\_Venda (1,1) — REFERENCIA — (0,n) Produto; Fornecedor (0,n) — FORNECE — (0,n) Produto; Fornecedor (0,n) — ATENDE — (1,1) Compra; Compra (1,n) — CONTEM — (1,1) Item\_Compra; Item\_Compra (1,1) — REFERENCIA — (0,n) Produto.
...
A entidade **Ordem\_Servico** representa o acompanhamento do serviço relacionado à venda, incluindo status e informações de previsão e entrega. Ela foi separada da Venda para não misturar o registro comercial da venda com o acompanhamento operacional do serviço.
